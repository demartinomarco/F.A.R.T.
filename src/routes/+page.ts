...
export function _extractPlatformNames(departures: ApiResponse) : string[] {
	const platformSet = [...new Set(departures.departureList.map((dep) => dep.platformName).filter(Boolean))]
...
export function _filterByPlatformName(departures: ApiResponse, selectedPlatforms: string[]) : Departure[] {
